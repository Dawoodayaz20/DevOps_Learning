...
10. `CMD ["executable","param1","param2"]`
  - *ENTRYPOINT* specifies the default execuatble too be run when the container starts. 
    1. CMD command param1 param2 e.g: CMD npm run dev
    2. It's like having a default dish for everey customer until someone specifically orders something else.
    3. ENTRYPOINT != CMD.
    ...
    4. If both have to be used, CMD's arguments will be passed to the ENTRYPOINT's arguments.
...
8. *Important Tip for Vite*:
  ...
  - So for that purpose, follow the steps below:
    * Edit your vite.config.ts file by doing this:
``` 
